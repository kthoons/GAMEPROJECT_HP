...
class Claw(pygame.sprite.Sprite):
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(screen, BLACK, self.position, 3) # 중심점 표시
...
backgorund = pygame.image.load(os.path.join(current_path, "background.png"))
...
claw = Claw(claw_image, (screen_width // 2, 110)) # 가로위치는 화면 가로 기준으로 절반, 세로위치는 위에서 110px
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN: # 마우스 버튼 누를 때 집게를 뻗음
            ...
            to_x = move_speed # move_speed 만큼 빠르게 쭉 벋음
